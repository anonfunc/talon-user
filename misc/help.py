...
from ..utils import optional_numerals, numerals
...
def render_contexts_help(_, target_page=1):
    ...
    for idx, context in enumerate(voice.talon.subs.values()):
        ...
        keymap.update({"help " + number: create_context_mapping(context)})
...
def normalize_trigger(trigger):
    trigger = trigger.replace(numerals, "##")
    trigger = trigger.replace(optional_numerals, "[##]")
    return trigger
...
def render_commands_webview(context, target_page=1):
    ...
    for trigger in context.triggers.keys():
        ...
        mapping.append((normalize_trigger(trigger), format_actions(actions)))
